...
(function() {
  ...
  app.run(function($templateCache) {
    ...
    $templateCache.put('/angular/jade-html/component/user/filters', "<!-- Application Icons--><!--mixin filter-icon--><!--  span.icon-Filter--><!-- Filter Icons--><!-- filter icons--><div ng-controller=\"Filters_Controller\"><div id=\"filters\" ng-repeat=\"filter in filters\"><section class=\"row__label\"><div class=\"label\"><icon class=\"Filter\"></icon><span class=\"text\"> \{\{filter.title}}</span></div><!--ul.display-inlineli a +close-icon--></section><section class=\"row\"><filter-item ng-repeat=\"result in filter.results\"><a ng-href=\"#\" ng-click=\"apply_Filter(result.id, result.title)\" ng-show=\"result.size &gt;0\" class=\"filter\"><div id=\"filter-icon\" ng-bind-html=\"result.icon\"></div><span class=\"text\">\{\{result.title}}</span><span class=\"badge\">\{\{result.size}}</span></a></filter-item></section></div></div>");
